export const formatImageUrl = (url?: string) => {
  ...
  // 2. Extraemos el Base URL de producción con fallback cableado por seguridad SSR
  const baseUrl = (import.meta.env.VITE_API_URL || 'https://admin.redgirdaccperu.edu.pe').replace(/\/+$/, "");

  // 3. INTERCEPCIÓN DE EMERGENCIA: Si trae fantasmas de Localhost, los fulminamos en el aire
  let sanitizedUrl = url;
  if (sanitizedUrl.includes('127.0.0.1:8000') || sanitizedUrl.includes('localhost:8000')) {
    sanitizedUrl = sanitizedUrl.replace(/^http:\/\/(127\.0\.0\.1|localhost):8000/, baseUrl);
  }
  ...
  // 7. Retorno definitivo unificado con el subdominio seguro HTTPS de producción
